...
function formatFieldValue(value: any, fieldName: string): any {
  ...
  if (
    fieldNamesConvertToHex.has(fieldName.toLowerCase()) &&
    Array.isArray(value)
  ) {
...
export async function convertProtobufToJson(
  buffer: ArrayBuffer,
  contentEncoding?: string
): Promise<string> {
  try {
    ...
    const messageTypes = [otelProto.opentelemetry.proto.trace.v1.TracesData]
    ...
    return JSON.stringify(
      {
        error:
          'Unable to parse as any known OpenTelemetry protobuf message type',
