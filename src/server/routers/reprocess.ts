  ...
  reprocessAll: workspaceProcedure.mutation(async ({ ctx }) => {
    ...
    for (const response of responsesToReprocess) {
      try {
        ...
        console.error(`[reprocess] Failed to reprocess ${response.id}:`, error)
        failureCount++
      }
    }
