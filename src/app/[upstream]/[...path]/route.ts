...
async function validateAuthKey(request: NextRequest): Promise<boolean> {
    if (process.env.DISABLE_AUTHENTICATION === 'true') {
        return true
    }
    const authHeader = request.headers.get('x-monitor-auth')
    ...
    try {
        ...
        console.error('Auth key validation error:', error)
        return false
    }
}
...
async function handleProxy(
    ...
    try {
        const isValidAuth = await validateAuthKey(request)
        if (!isValidAuth) {
            return NextResponse.json(
                {error: 'Unauthorized: Invalid or missing API key. Use X-Auth header'},
                {status: 401}
            )
        }
